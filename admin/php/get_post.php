...
if($_REQUEST['switch']=="post"){
	...
	$sql = "select * FROM post where post_id = ".$_REQUEST['pid'];
	...
	$sclass='post_sav';
	if($myData['save']=='1'){$ps = 0;$sclass='post_sav2';}
    ...
    $post_keep = '<li onclick="post_delete('.$row['post_id'].')" id="keep">	
                					
                					<span>Keep</span>
                				</li>';
	//echo
		echo('
		<table align="center" border="0" class="post_table" id="post_'.$row['post_id'].'">
            <tr>
                <td colspan="4" class="post_title_bar">
                	...
                	<div onclick="post_option('.$row['post_id'].')" style="flex-flow: row-reverse;display: flex;">
                		<span style="font-size: medium;" class="material-icons post_option_ico">delete_forever</span>
